...
class NSimplex (object):
    ...
    def _get_apex(cls, base, distances):
        """ Find new apices given a `base`generated by the pivots {p1, .., pN} and the `distances` of the input object to the pivots {p1, .., pN}.
        This function works on batches of inputs.

        Args:
            base (torch.tensor): a (N,N-1)-shaped tensor representing the simplex base.
            distances (torch.tensor): a (B,N)-shaped tensor containing distances to the P pivots for B objects.

        Returns:
            torch.tensor: a (B,N)-shaped tensor containing the new apices.
        """
    ...
    def build_base(self, distances, progress=False):
        ...
        Args:
            distances (torch.tensor): a (n,n)-shaped tensor containing distances between pivots.
            progress (bool, optional): if True, show progress with tqdm. Defaults to False.

        TODO: change to accept also (½x n x(n-1))-shaped array containing linearized values of the
    ...
    def embed(self, distances):
        ...
        Args:
            distances (torch.tensor): a a (n,)- or (B,n)-shaped tensor containing the distance
                                      to pivots of the objects to embed.

        Returns:
            torch.tensor: a (B,b)-shaped tensor containing the embedding. B=1 if input was an 1D-tensor.
        """
    ...
    def estimate(self, x, y, kind='zenit'):
        ...
        Args:
            x (torch.tensor): a a (n,)- or (B,n)-shaped tensor of embeddings
            y (torch.tensor): a a (n,)- or (B,n)-shaped tensor of embeddings
            kind (str, optional): Kind of estimation. Can be one of 'lower', 'upper', or 'zenit'. Defaults to 'zenit'.

        Returns:
            torch.tensor: a (B,)-shaped tensor containing distance estimantes.
        """
